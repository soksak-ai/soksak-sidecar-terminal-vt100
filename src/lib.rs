//! soksak-sidecar-terminal-vt100 — 라이브러리 면.
//!
//! `soksak-spec-sidecar-terminal@1` 계약(정본 = soksak-contract-terminal)의 **vt100 엔진
//! 구현**. 도메인 로직(복원 미러·직렬화기)과 엔진 좌석을 모듈로 가른다:
//!   [`engine`]  vt100 을 만지는 유일한 모듈(엔진-중립 뷰만 노출) — 미러의 피고 엔진.
