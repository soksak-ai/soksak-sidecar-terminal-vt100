...
//! 계약(soksak-spec-sidecar-terminal@1 SPEC.md §4·§6)이 문서화한 바이트를 그대로 구현한다.
//! 여기 상수·경로 파생·프레임 포맷이 데몬(soksak-ptyd)의 것과 어긋나면 통합이 깨진다 —
//! 정본은 데몬 crate, 이 파일은 그 계약의 소비자 구현이다.
//!
...
pub const CLIENT_ID: &str = "soksak-sidecar-terminal-vt100";
...
// 싱글턴이 계약당 하나이도록 엔진 이름을 넣지 않는다 — 모든 터미널 엔진 유닛이 같은 경로를
