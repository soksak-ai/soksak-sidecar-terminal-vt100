...
contract the other engine units implement on their own engines. One contract, many engine
units, one at a time behind a terminal plugin's manifest declaration (NAMING §8: the engine
is in the unit's name). vt100 is in this one's because the engine is what makes it a
separate unit — two units implementing this contract differ in nothing else a caller can
see.
