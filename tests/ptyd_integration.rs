...
//!
//! 데몬 wire 는 엔진-불가지라 vt100 미러도 같은 tee→feed→rehydrate/StoreBlob 왕복을 탄다.
//!
//! 다섯 결(전부 실 데몬):
//!   1. library_tee_consumption_and_seal — ControlClient/TeeStream/Mirror/store_blob 를
...
//!   3. ensure_session — 부팅 후 태어난 세션의 동적 구독.
//!   4. late-subscribe seed — 구독 '전' 출력도 warm 복원에 담긴다(링 씨앗).
//!   5. boot-order — 데몬보다 먼저 뜬 서비스도 결국 봉인-블롭을 쓴다.
...
use soksak_sidecar_terminal_vt100::daemon::{ControlClient, TeeFrame, TeeStream};
use soksak_sidecar_terminal_vt100::mirror::Mirror;
use soksak_sidecar_terminal_vt100::proto;
use soksak_sidecar_terminal_vt100::service::ServiceClient;
...
fn fresh_home(tag: &str) -> PathBuf {
    ...
    let home = PathBuf::from(std::env::var("HOME").unwrap())
        .join(".soksak-e2e")
        .join(format!("vt-{tag}-{}-{nanos}", std::process::id()));
...
fn sidecar_bin() -> String {
    std::env::var("SOKSAK_SIDECAR_BIN")
        .ok()
        .filter(|p| !p.is_empty())
        .unwrap_or_else(|| env!("CARGO_BIN_EXE_soksak-sidecar-terminal-vt100").to_string())
}
