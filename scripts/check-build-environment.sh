...
case "$(uname -s)-$(uname -m)" in
  ...
  Darwin-x86_64) if [ "$(sysctl -n hw.optional.arm64 2>/dev/null || true)" = 1 ]; then host_target=aarch64-apple-darwin; node_platform=darwin; node_arch=arm64; else host_target=x86_64-apple-darwin; node_platform=darwin; node_arch=x64; fi ;;
...
if [ "$target" != "$host_target" ] || [ -z "$rust_expected" ] || [ "$rust_actual" != "$rust_expected" ] || [ "$rust_host" != "$target" ] || [ "$node_actual_platform" != "$node_platform" ] || [ "$node_actual_arch" != "$node_arch" ]; then
  printf 'TOOLCHAIN_MISMATCH: target=%s hostTarget=%s rust=%s/%s nodeRuntime=%s/%s; expected rust=%s/%s nodeRuntime=%s/%s\n' "$target" "$host_target" "${rust_actual:-missing}" "${rust_host:-unknown}" "${node_actual_platform:-unknown}" "${node_actual_arch:-unknown}" "$rust_expected" "$target" "$node_platform" "$node_arch" >&2
  exit 78
fi
