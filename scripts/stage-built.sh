...
mkdir -p "$out"; [ ! -L "$out" ] || { echo 'stage output must not be a symbolic link' >&2; exit 2; }; staged=$name$ext
if [ -e "$out/$staged" ]; then cmp -s "$source" "$out/$staged" || { echo 'staged binary conflicts with current build' >&2; exit 1; }
else cp "$source" "$out/.$staged.next.$$"; chmod +x "$out/.$staged.next.$$"; mv "$out/.$staged.next.$$" "$out/$staged"; fi
generated=$out/.sidecar.json.next.$$
sed "s#\"process\": \"dist/$name\"#\"process\": \"dist/$staged\"#" sidecar.json > "$generated"
if [ -e "$out/sidecar.json" ]; then cmp -s "$generated" "$out/sidecar.json" || { echo 'staged manifest conflicts with source' >&2; exit 1; }; find "$generated" -delete
else mv "$generated" "$out/sidecar.json"; fi
